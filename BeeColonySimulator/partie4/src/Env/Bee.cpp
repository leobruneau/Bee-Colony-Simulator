...
void Bee::drawOn(sf::RenderTarget &target) const {
    auto beeSprite(buildSprite(getPosition(), getRadius(), getAppTexture(getAppConfig().scout_texture)));
...
void Bee::update(sf::Time dt) {
    ...
    if (getAppEnv().isFlyable(newPosition)) move(dt);
    else {
        double beta(.0);
...
void Bee::move(sf::Time dt) {
        Collider::move(velocity_*dt.asSeconds());
}
